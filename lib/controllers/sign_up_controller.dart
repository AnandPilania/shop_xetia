...
class SignUpController extends GetxController {
  ...
  void resEmailVerify({@required BuildContext context}) async {
    ...
    loading.show();
    await authV2
        .verifyEmail(email: _textFieldController.emailSignUp.text, verifyT: _textFieldController.token.text)
        .then((AuthResponse value) {
      loading.hide();
      if (value.meta.code == 200) {
        Get.snackbar('Alert', value.meta.message, colorText: context.theme.primaryColorLight);
      ...
      loading.hide();
      Get.snackbar('Alert', "Email Verify Failed", colorText: context.theme.primaryColorLight);
      print(onError);
    });
  }
  ...
  void resRequestEmailVerify({@required BuildContext context}) async {
    loading = LoadingOverlay.of(context);
    loading.show();
    await authV2.reqVerifyEmail(email: _textFieldController.resendEmail.text).then((AuthResponse value) {
      loading.hide();
      if (value.meta.code == 200) {
        FocusScope.of(context).unfocus();
