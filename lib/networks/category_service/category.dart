...
class Category {
  ...
  Future<ListCategoryResponse> listGetCategory() async {
    try {
      http.Response res = await http
          .get("$baseUrl/api/v1/category/get-data?limit=10&page=1")
          .timeout(const Duration(seconds: 10), onTimeout: () {
        throw TimeoutException("connection time out try agian");
      });
      ;
  ...
  Future<CategoryResponse> putCategory() async {
    try {
      http.Response res = await http
          .put(
              "$baseUrl/api/v1/category/update/d6aa8c3f-4145-49dc-8519-cd82a96fba98",
              headers: {
                "Accept": "application/json",
                "Content-Type": "application/json",
                "Authorization":
  ...
  Future<CategoryResponse> deleteCategory() async {
    try {
      http.Response res = await http.delete(
          "$baseUrl/api/v1/category/delete/d6aa8c3f-4145-49dc-8519-cd82a96fba98",
          headers: {
            "Accept": "application/json",
            "Content-Type": "application/json",
            "Authorization":
